...
\textbf{Definizione (\(T_{batch}\)):} Sia
\[
  T_{batch} = \SI{24}{\hour}
\]
...
\textbf{Definizione (\(N_{batch}\)):} Sia
\[
  N_{batch} = \frac{T_{batch}}{\Delta t_{\text{slot}}}
\]
...
\textbf{Definizione (\(\bigl(B_{\text{id},k}\bigr)_{k=1}^{N_{batch}}\)):} Sia
\[
  \mathbf{B}
  = \bigl(B_{\text{id},k}\bigr)_{k=1}^{N_{batch}},
  \qquad B_{\text{id},k} \in \{0,1\}^{128},\ \forall k \in \{1,\dots,N_{batch}\}.
\]
una sequenza ordinata di identificativi $B_{\text{id}}$

Il client trasmittente mantiene in memoria locale un batch di identificativi \(B_{\text{id}}\) fornito dal backend, valida per $N_{batch}$ di $S_{slot}$ consecutivi.

\textbf{Definizione (\(S_j\)):} Sia l'indice del primo slot del batch \(S_1 \in \mathbb{N}\) con
\[
  S_j = S_1 + (j-1), \qquad \forall j \in \{1,\dots,N_{batch}\},
\]
...
\begin{enumerate}
  \item Il backend genera un batch di identificativi
        \[
          \mathbf{B} = \bigl(B_{\text{id},j}\bigr)_{j=1}^{N_{batch}}
        \]
        e l'indice del primo slot del batch \(S_1\), e li invia al client trasmittente.

  \item Il client memorizza localmente \(\mathbf{B}\) e \(S_1\), validi per \(N_{batch}\) slot temporali consecutivi.
  ...
  \item Se invece \(j^\ast < 1\) oppure \(j^\ast > N_{batch}\), il batch corrente non copre più lo slot attuale e il client deve ottenere un nuovo batch dal backend eliminando quello attuale.
\end{enumerate}
...
\[
  \mathbf{B}
  = \bigl(B_{\text{id},j}\bigr)_{j=1}^{N_{batch}},
...
soddisfa
\[
  S_1 \le S_{\text{slot}}^{\text{curr}} \le S_1 + N_{batch} - 1.
\]
...
Al primo avvio il client non dispone di un batch valido, richiede immediatamente al backend un nuovo batch che copre le prossime $T_{batch}$

\subsubsection{Batch scaduto}
Se il batch corrente è scaduto, cioè
\[
  S_{\text{slot}}^{\text{curr}} > S_1 + N_{batch} - 1
\]
...
In condizioni normali, finché esiste un batch valido e l'orologio del dispositivo resta sufficientemente sincronizzato, l'app può operare in modo autonomo per una durata pari a $T_{batch}$, riallineandosi al backend non appena necessario.
...
\begin{itemize}
  ...
  \item l’impatto della compromissione di un device è quindi limitato alla possibilità di replay
        ...
        \(T_{batch}\), ed è considerato accettabile rispetto al beneficio di autonomia del client.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item non distribuiamo chiavi ai device: il client riceve direttamente i \(B_{\text{id}}\)
        necessari per un orizzonte \(T_{batch} = \SI{24}{\hour}\), senza conoscere \(K_i\);
...
\begin{itemize}[nosep]
  ...
  \item introduce il batch di durata \(T_{batch} = \SI{24}{\hour}\) inviato al client, che riceve
        una sequenza
        \(\mathbf{B} = (B_{\text{id},j})_{j=1}^{N_{batch}}\) e può operare offline finché il batch
        resta valido;
...
\begin{itemize}
  ...
  \item Per ogni utente, il backend genera periodicamente un batch
        \[
          \mathbf{B} = \bigl(B_{\text{id},j}\bigr)_{j=1}^{N_{batch}},
          \qquad N_{batch} = \frac{T_{batch}}{\Delta t_{\text{slot}}},
        \]
  ...
  \item Il client trasmittente conserva localmente \(\mathbf{B}\) e \(S_1\) e, all’istante \(t\),
        ...
        \[
          j^\ast = S_{\text{slot}}^{\text{curr}} - S_1 + 1,
        \]
        usando \(B_{\text{id},j^\ast}\) se e solo se \(1 \le j^\ast \le N_{batch}\).
...
\begin{itemize}
  ...
  \item \textbf{Assenza di chiavi lato device}: i client non ricevono mai \(K_i\); conservano solo
        ...
        generazione di nuovi valori al di fuori dell’orizzonte \(T_{batch}\).
